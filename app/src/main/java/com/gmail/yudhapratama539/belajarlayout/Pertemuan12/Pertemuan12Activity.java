...
public class Pertemuan12Activity extends AppCompatActivity {
    ...
    public void tampilkanData() {
        try {
            ...
            while (input.available() > 0) {
                input.read(bufNama); input.read(bufTelepon);
                String dataNama = "";
                ...
                Log.d("Data", dataNama+", "+dataTelepon);
    ...
    public void tampilkanDataJson() {
        try {
            ...
            while (input.available() > 0) {
                ...
                //infoData = infoData + no + " " + dataNama + " - " + dataTelepon + "\n";
                Log.d("Data", dataNama+", "+dataTelepon);
                User user1 = new User(dataNama, dataTelepon);
                user.add(user1);
                //no++;
            }
            //menampilkan data ke teks view
            infoData = infoData + new Gson().toJson(user);
            dataTelepon.setText(infoData);
            dataFile.close();
        } catch (IOException e) {
            Toast.makeText(getBaseContext(), "Kesalahan: " + e.getMessage(), Toast.LENGTH_LONG).show();
